...
| 🎨 **界面主题** | **默认炭金·深色（静态）**：深炭灰底 `#1A2430` + 米白正文 `#FAFAF8` + 金棕点缀 `#D4A017`（标题边框 / 分割线 / 卡片标签，**实测仅占 0.05% 像素 —— 极克制**）+ 冷灰蓝次要文字 `#8FA6B2` + 语义绿 `#4CAF50` / 红 `#E53935`。定位：深色基底 + 低饱和金点缀，**高级克制，面向高净值客户**。实测正文对比度 **10.8:1**。顶栏可一键切另外两套：**浅色**（TypeSafe AI：`#fefefe` 纸面 + `#1e1e1e` 近黑 + 品红 `#d45bb6` + 虚线描边）、**赛博**（`#000` + 霓虹绿 `#00ff41`）。选择记在浏览器里 |
...
|---|---|---|
| `--bg` | `#1A2430` | 深炭灰底（默认） |
| `--text` | `#FAFAF8` | 米白正文（对比度 10.8:1） |
| `--accent` | `#D4A017` | 金棕点缀提亮（深底上；仅占 0.05% 像素） |
| `--muted` | `#8FA6B2` | 冷灰蓝提亮（深底上约 6:1） |
| `--green` / `--red` | `#4CAF50` / `#E53935` | 深底提亮版（原色在深底仅 3:1） |
| `--border` | `rgba(184,134,11,.30)` | 金棕细线（点缀，不铺满） |
| `--panel` | `#212D3B` | 卡片：比底色亮一层 |
| `--line` | `rgba(250,250,248,.07)` | 48px 网格线（中性淡色，金色只留给重点） |
...
**赛博主题**（顶栏循环切换 `◆ 炭金·深 → ◈ 炭金·浅 → 🌙 赛博`）保留霓虹绿终端配色
...
- 主题：`?theme=tandark`（默认，炭金深色）/ `?theme=tan`（炭金米白）/ `?theme=cyber`；动效：`?motion=on|off`（默认 off）
