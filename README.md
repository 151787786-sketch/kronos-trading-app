...
| 🎨 **界面主题** | **默认炭金（米白底，静态）**：米白页面底 `#FAFAF8` + 深炭灰正文 `#1A2430` + 金棕点缀 `#B8860B`（标题边框 / 分割线 / 卡片标签，**少量**）+ 冷灰蓝次要文字 `#546E7A` + 语义绿 `#2E7D32` / 红 `#C62828`。定位：深色基底 + 低饱和金点缀，**高级克制，面向高净值客户**。实测正文对比度 **11.4:1**。顶栏可一键切另外两套：**浅色**（TypeSafe AI：`#fefefe` 纸面 + `#1e1e1e` 近黑 + 品红 `#d45bb6` + 虚线描边）、**赛博**（`#000` + 霓虹绿 `#00ff41`）。选择记在浏览器里 |
...
| 变量 | 值 | 用途 |
|---|---|---|
| `--bg` | `#FAFAF8` | 米白页面底（替代纯白，降低刺眼感） |
| `--text` | `#1A2430` | 深炭灰正文（对比度 11.4:1） |
| `--accent` | `#B8860B` | 金棕点缀（标题边框 / 分割线 / 卡片标签，少量） |
| `--muted` | `#546E7A` | 冷灰蓝次要文字 |
| `--green` / `--red` | `#2E7D32` / `#C62828` | 收益 / 风险 |
| `--border` | `rgba(184,134,11,.34)` | 金棕细线（点缀，不铺满） |
| `--panel` | `#FFFFFF` | 卡片：纯白浮于米白之上 |
| `--line` | `rgba(26,36,48,.07)` | 48px 网格线（中性淡色，金色只留给重点） |
...
- 主题：`?theme=tan`（默认炭金米白）/ `?theme=tandark`（炭金深色）/ `?theme=cyber`；动效：`?motion=on|off`（默认 off）
