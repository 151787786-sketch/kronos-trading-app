...
| 🎨 **界面主题** | **默认深灰底 + 橙色文字（静态琥珀终端）**：深灰底 `#1c1c1c` + 琥珀橙正文 `#ffb454` + 亮橙强调 `#ff8c1a` + 暖黄次强调 `#ffd166` + 橙色细线网格 `rgba(255,140,26,.20)` + 直角。实测正文对比度 **6.6:1**（过 WCAG AA）。顶栏可一键切另外两套：**浅色**（TypeSafe AI：`#fefefe` 纸面 + `#1e1e1e` 近黑 + 品红 `#d45bb6` + 虚线描边）、**赛博**（`#000` + 霓虹绿 `#00ff41`）。选择记在浏览器里 |
...
| 变量 | 值 | 用途 |
|---|---|---|
| `--bg` | `#1c1c1c` | 深灰底（不用纯黑，眼睛更舒服） |
| `--text` | `#ffb454` | 琥珀橙正文（对比度 6.6:1） |
| `--accent` | `#ff8c1a` | 亮橙强调 |
| `--accent-2` | `#ffd166` | 暖黄次强调 |
| `--muted` | `#a8783f` | 暗橙次要文字 |
| `--border` | `rgba(255,140,26,.20)` | 橙色细线（科技感来源） |
| `--panel` | `rgba(36,36,36,.94)` | 面板底（深灰） |
| `--line` | `rgba(255,140,26,.13)` | 40px 网格线 |
