...
| 🎨 **界面主题** | **默认 SharedNet 风（浅蓝白 + 牛津深蓝，静态）**：色板取自 [sharednet.ai](https://www.sharednet.ai/)——浅蓝白底 `#F3F7FF`（原站 `oklch(96.8% .025 240)`）+ 牛津深蓝正文 `#002147` + 中蓝强调 `#205F91` + 陶土橙点缀 `#D97757` + 石板蓝次要文字 `#3F5A79` + 装饰浅蓝 `#90CAF9`/`#64B5F6`/`#0077B6`。字体栈含原站 Albert Sans / Anybody。正文对比度约 **15:1**。顶栏可一键切另外两套：**浅色**（TypeSafe AI：`#fefefe` 纸面 + `#1e1e1e` 近黑 + 品红 `#d45bb6` + 虚线描边）、**赛博**（`#000` + 霓虹绿 `#00ff41`）。选择记在浏览器里 |
...
| 变量 | 值 | 用途 |
|---|---|---|
| `--bg` | `#F3F7FF` | 浅蓝白底（原站 `oklch(96.8% .025 240)`） |
| `--text` | `#002147` | 牛津深蓝正文（原站 hex，约 15:1） |
| `--accent` | `#205F91` | 中蓝强调（原站） |
| `--muted` | `#3F5A79` | 石板蓝次要文字（原站 `oklch(46% .06 252)`） |
| `--green` / `--red` | `#17805A` / `#C8452C` | 原站 oklch 绿 / 红 |
| `--border` | `rgba(32,95,145,.22)` | 中蓝细线 |
| `--panel` | `#FFFFFF` | 卡片：纯白浮于浅蓝白之上 |
| `--line` | `rgba(32,95,145,.10)` | 72px 网格线 |
...
> **想再调色**：改 `:root` 里的 `--bg` / `--text` / `--accent` / `--border` 四个值即可整站换肤。
...
**赛博主题**（顶栏循环切换 `◉ 共享网络 → ◆ 炭金·深 → ◈ 炭金·浅 → 🌙 赛博`）保留霓虹绿终端配色
（`#000` 底 + `#00ff41` 霓虹绿 + `#d7e3db` 骨白）。三套主题共用同一批语义变量，切换零成本。

```html
<!-- URL 直达 -->
?theme=tandark  （默认，炭金深色底）
?theme=tan      （炭金米白底）
?theme=cyber    （霓虹绿赛博）
...
> 深灰 `#ff4d6a` 红 / `#2ee6a8` 绿（避开橙色，防止和正文混淆），浅色 `#d92b1f` / `#0f8a4d`，赛博 `#ef4444` / `#22c55e`。
...
- 主题：`?theme=sn`（默认，SharedNet 风）/ `?theme=tandark`（炭金深色）/ `?theme=tan`（炭金米白）/ `?theme=cyber`（赛博）；动效：`?motion=on|off`（默认 off）
