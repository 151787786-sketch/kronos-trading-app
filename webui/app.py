...
# Prefer local model weights if present (e.g. D:\kronos\models\<ModelName>),
# otherwise fall back to downloading from the Hugging Face Hub at first load.
# Set HF_ENDPOINT=https://hf-mirror.com to use the HF mirror for downloads.
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_LOCAL_MODELS_DIR = os.environ.get("KRONOS_MODELS_DIR", os.path.join(_PROJECT_ROOT, "models"))


def _resolve_local_model(repo_id):
    """Return a local directory path if the model was pre-downloaded, else repo_id."""
    local_dir = os.path.join(_LOCAL_MODELS_DIR, os.path.basename(repo_id))
...
def load_model():
    ...
    try:
        ...
        # Load tokenizer and model (use local weights when pre-downloaded)
        tokenizer = KronosTokenizer.from_pretrained(_resolve_local_model(model_config['tokenizer_id']))
        model = Kronos.from_pretrained(_resolve_local_model(model_config['model_id']))
